...
from FindRegion import GetCFsName, update_series, update_timenum, update_series_ERA5, update_series_ERA52
...
DemandName, SolarCFsName, WindCFsName = GetCFsName(region, 'ERA5')
...
tech_list[solar_idx]['series_file']  = SolarCFsName[1]; update_series_ERA52(case_dic, tech_list[solar_idx], SolarCFsName)
tech_list[wind_idx]['series_file']   = WindCFsName[1];  update_series_ERA52(case_dic, tech_list[wind_idx], WindCFsName)
...
for idx in range(len(co2_constraints_list)):
    ...
    case_dic['case_name'] = case_name_default + '222_Year' + str(year) + '_' + str(region) + '_Co2Cons' + str(co2_constraints_percentage[idx])
