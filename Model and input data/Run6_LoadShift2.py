...
from FindRegion2 import GetCFsName, update_series, update_timenum
...
for idx in range(len(tech_list)):
    ...
    if name == 'shift_load': shift_load_idx = idx
...
for idx in range(len(unmet_demand_cost)):
    tech_list[shift_load_idx]['var_cost'] = unmet_demand_cost[idx]
    case_dic['case_name'] = case_name_default + '_Year' + str(year) + '_' + str(region) + '_LoadShift' + str(unmet_demand_cost[idx])
