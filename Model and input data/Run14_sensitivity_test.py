...
from FindRegion import GetCFsName2, update_series, update_timenum
...
### Set cycle values
co2_constraints_percentage = np.array([1e24, 98, 96, 94, 92, 90, 88, 86, 84, 82, 80, 78, 76, 74, 72, 
                                         70, 68, 66, 64, 62, 60, 58, 56, 54, 52, 50, 48, 46, 44, 42, 
                                         40, 38, 36, 34, 32, 30, 28, 26, 24, 22, 20, 18, 16, 14, 12, 
                                         10, 8,  6,  4,  2,  1, 0.1, 0.01, 0.001, 0])
# co2_constraints_percentage = np.array([4,  2,  1, 0.1, 0.01, 0.001, 0])

region1 = 'US'
### Set basic information
case_dic['output_path'] = '/data/carnegie/leiduan/cesm_archive/MEM_AdvNuc'
DemandName, SolarCFsName, WindCFsName = GetCFsName2(region1, region2)
...
for idx in range(len(co2_constraints_list)): 
    ...
    case_dic['case_name'] = case_name_default + '_Year' + str(year) + '_' + str(region1) + '_' + str(region2) + '_Co2Cons' + str(co2_constraints_percentage[idx])
